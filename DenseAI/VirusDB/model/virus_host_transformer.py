...
from keras.layers import Embedding, Softmax, Dropout, Conv1D, Lambda, Flatten, Dense, Add, GlobalAveragePooling1D, BatchNormalization
...
from DenseAI.VirusDB.transformer.position import TransformerCoordinateEmbedding
from DenseAI.VirusDB.transformer.transformer import TransformerACT, TransformerBlock
from DenseAI.VirusDB.transformer.transformer import gelu
from DenseAI.VirusDB.transformer.extras import ReusableEmbedding, TiedOutputEmbedding
from DenseAI.VirusDB.common.utils import load_dictionary
...
class Transformer_Segmenter:
    ...
    def __build_model(self):
        ...
        # 辅助编号， B I E S
        segment_ids = Input(shape=(self.max_seq_len,), dtype='int32', name='segment_ids')
        ...
        #     self.ws_num_classes,              # B I E S embeddings
        #     self.embedding_size_word,         # word_embedding_size,
        ...
        # segment_project_layer = Conv1D(self.model_dim, activation="linear",
        #                            kernel_size=1,
        #                            name="segment_project")
        # segment_dropout_layer = Dropout(self.embedding_dropout, name="segment_dropout")
        # segment_embeddings = segment_project_layer(segment_dropout_layer(segment_embeddings))
        ...
        #next_step_input = Capsule(num_capsule=20, dim_capsule=20, routings=4, share_weights=True)(next_step_input)
        #next_step_input = Flatten()(next_step_input)
        #next_step_input = BatchNormalization()(next_step_input)
        #next_step_input = Dropout(0.25)(next_step_input)


        #next_step_input = GlobalAveragePooling1D()(next_step_input)
        class_prediction = (
            Dense(self.tgt_vocab_size, name='class_prediction', activation='softmax')
            (next_step_input))
        ...
        parallel_model.compile(optimizer=self.optimizer, loss=[sparse_categorical_crossentropy], metrics=['accuracy'])

        return model, parallel_model


    def decode_sequences(self, sequences, label_dict):
        sequences = self._seq_to_matrix(sequences)
        # segment_ids = self._seq_to_matrix(segment_ids)
        output = self.model.predict_on_batch([sequences])  # [N, -1, chunk_size + 1]

        return output

    def decode_texts(self, texts, sequences, label_dict):
        output = self.decode_sequences(sequences, label_dict)
        return output
